...
async def register_agent(data: AgentRegisterRequest, session: AsyncSession = Depends(get_session)):
    try:
        ...
        app = await ApplicationService.create_application(session, data.full_name, data.max_user_id, data.phone, data.email, data.city, RegistrationType(data.registration_type))
